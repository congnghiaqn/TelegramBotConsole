    ...
    public class BotHandler
    {
        ...
        private static async Task BotOnMessageReceived(ITelegramBotClient botClient, Message message)
        {
            ...
            var action = message.Text.Split(' ').First() switch
            {
                ...
                "/poll" => SendPolling(botClient, message),
                _ => Usage(botClient, message)
            };
            Message sentMessage = await action;
            Console.WriteLine($"The message was sent with id: {sentMessage.MessageId}");

            static async Task<Message> SendPolling(ITelegramBotClient botClient, Message message)
            {
                await botClient.SendChatActionAsync(message.Chat.Id, ChatAction.Typing);
                await Task.Delay(400);
                return await botClient.SendPollAsync(
                    chatId: message.Chat.Id,
                    question: "Which ARTS game is the best?",
                    openPeriod: 300,
                    isAnonymous: true,
                    options: new[]
                    {
            ...
            static async Task<Message> SendInlineKeyboard(ITelegramBotClient botClient, Message message)
            {
                ...
                InlineKeyboardMarkup inlineKeyboard = new(
                    ...
                    new[]
                    {
                        ...
                        InlineKeyboardButton.WithSwitchInlineQueryCurrentChat("switch inline current chat query")
                    },
                    });
